...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const questions = await prisma.question.findMany({ where: { id: { in: attempt.questionOrder } }, select: { id: true, text: true, optionA: true, optionB: true, optionC: true, optionD: true } });
